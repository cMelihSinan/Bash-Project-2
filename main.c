...
int main(void) {
    //Hold the command entered with Buffer
    ...
    while (1) {
        //For each command reset file descriptors
        ...
        if (args[0] == NULL) {
            continue; //If there is no command restart
        }
...
//function to find a command in the system's PATH
char* find_command(const char *command) {
...
//A signal handler for the SIGTSTP signal
void sigtstp_handler(int sig) {

    //foreground process running check
    if (foreground_pid > 0) {
...
void exit_shell() {
    //background processes running check
...
//function to list all stored bookmarks
void list_bookmarks() {
...
//function to execute a command stored in bookmarks list by its index
void execute_bookmark(int index) {
...
//function to delete a bookmark from the bookmarks list based on its index
void delete_bookmark(int index) {
...
//function to handle input/output redirection in a command
void handle_redirection(char *args[], int *in_fd, int *out_fd, int *err_fd) {
